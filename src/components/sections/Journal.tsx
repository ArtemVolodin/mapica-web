    ...
    <section id="journal" className="relative py-24 md:py-32 scroll-mt-section">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          ...
          description="Notes on AI-native workflows, faster MVPs and shipping premium mobile products."
        />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {journalArticles.map((post, i) => (
            <motion.article
              key={post.slug}
              className="group premium-card glass rounded-2xl overflow-hidden border border-white/8 hover:border-white/15 transition-all duration-400 flex flex-col"
              initial={false}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.08 }}
              whileHover={{ y: -6 }}
              onHoverStart={() => setHovered(post.slug)}
              onHoverEnd={() => setHovered(null)}
            >
              <div className="premium-card-shine" />
              <div
                className={cn(
                  "relative h-36 bg-gradient-to-br transition-transform duration-500",
                  post.gradient,
                  hovered === post.slug && "scale-[1.02]"
                )}
              >
                ...
                <motion.p
                  className="absolute bottom-3 left-3 right-3 text-xs text-zinc-300/90 line-clamp-2"
                  initial={{ opacity: 0, y: 6 }}
                  animate={
                    hovered === post.slug ? { opacity: 1, y: 0 } : { opacity: 0, y: 6 }
                  }
                >
                  {post.excerpt}
                </motion.p>
